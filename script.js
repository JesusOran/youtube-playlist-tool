  ...
  async fetchVideoIds() {
      try {
          ...
          while (nextPageToken !== undefined) {
              const response = await this.makeApiCall(`https://www.googleapis.com/youtube/v3/playlistItems?part=contentDetails,status&maxResults=50&playlistId=${playlistId}&key=${this.apiKey}&pageToken=${nextPageToken}`);
              if (!response.ok) throw new Error(`Error: ${response.statusText}`);
              const data = await response.json();
      
              if (data.items) {
                  data.items.forEach(item => {
                      if (item.status.privacyStatus === 'private' || item.status.privacyStatus === 'unlisted' || item.status.privacyStatus === 'public') {
                      ...
                      this.updateOutputTextarea(`Available Video IDs: ${JSON.stringify(this.availableVideoIds, null, 2)}\nUnavailable Video IDs: ${JSON.stringify(this.unavailableVideoIds, null, 2)}`);
                  });
              }
  ...
  async fetchVideoDurations() {
      try {
          ...
          for (const chunk of videoIdChunks) {
              const response = await this.makeApiCall(`https://www.googleapis.com/youtube/v3/videos?part=contentDetails&id=${chunk.join(',')}&key=${this.apiKey}`);
              if (!response.ok) throw new Error(`Error: ${response.statusText}`);
              const data = await response.json();
  
              if (data.items) {
                  data.items.forEach(item => {
                      let duration = this.convertISO8601ToSeconds(item.contentDetails.duration);
                      ...
                      // Update the textarea in real-time
                      this.updateOutputTextarea(JSON.stringify(videoDetailsArray, null, 2));
                  });
              }
          }
          ...
          this.updateOutputTextarea(`Error fetching video durations: ${error.message}`);
  ...
  createMasterPlaylist(videoDetailsArray) {
      ...
      for (const video of videoDetailsArray) {
          ...
          currentPlaylist.push({
              ...
              endTime: currentPlaylist.length === 0 ? video.endTime - video.startTime : currentTime + (video.endTime - video.startTime)
          });
      ...
      const averageLength = Math.floor(masterPlaylist.reduce((sum, list) => sum + list.length, 0) / masterPlaylist.length);
      ...
      if (lastPlaylist.length < averageLength) {
          ...
          while (lastPlaylist.length < averageLength && allOtherVideos.length > 0) {
              ...
              lastPlaylist.push({
                  ...
                  endTime: lastVideoEndTime + (randomVideo.endTime - randomVideo.startTime)
              });
          }
      }
...
masterButton.addEventListener("click", async () => {
  ...
  if (ytPlaylist.validateInputs()) {
      ...
      const masterPlaylist = ytPlaylist.createMasterPlaylist(shuffledVideoDetails);
...
downloadFileButton.addEventListener("click", () => {
  const blob = new Blob([`export const masterPlaylist = ${outputTextarea.value};`], { type: 'application/javascript' });
